...
class IssueAlterationScreen extends StatefulWidget {
  const IssueAlterationScreen({super.key});

  @override
  State<IssueAlterationScreen> createState() => _IssueAlterationScreenState();
}

class _IssueAlterationScreenState extends State<IssueAlterationScreen> {
  String selectedName = '';
  String slNo = 'AL0001';
  String currentDate = '';
  ...
  Future<void> _generateSlNo() async {
    final prefs = await SharedPreferences.getInstance();
    int lastNo = prefs.getInt('last_alteration_sl_no') ?? 0;
    lastNo++;
    setState(() {
      slNo = 'AL${lastNo.toString().padLeft(4, '0')}';
    });
  }
  ...
  Future<void> _saveBill() async {
    ...
    try {
      ...
      await prefs.setInt('last_alteration_sl_no', currentNo);

      // Create alteration bill object
      final bill = {
        'slNo': slNo,
        'date': currentDate,
      ...
      // Load existing alteration bills
      List<String> bills = prefs.getStringList('alteration_bills') ?? [];
      bills.add(jsonEncode(bill));
      await prefs.setStringList('alteration_bills', bills);

      // Reduce inventory quantities and weights
      await _updateInventoryQuantities();

      // Update account weight (subtract pure weight)
      await _updateAccountWeight();

      _showSnackBar('Alteration bill saved successfully!', Colors.green);
  ...
  Future<void> _updateAccountWeight() async {
    ...
    if (accountsJson != null && accountsJson.isNotEmpty) {
      try {
        ...
        print('Total pure weight to subtract: $totalPureWeight');
        ...
        if (accountIndex != -1) {
          ...
          var currentWeightValue = accounts[accountIndex]['weight'];
          double currentWeight =
              double.tryParse(currentWeightValue?.toString() ?? '0') ?? 0.0;

          print('Current weight for $selectedName: $currentWeight');

          // Subtract pure weight (ALLOW NEGATIVE VALUES)
          double newWeight = currentWeight - totalPureWeight;

          print('New weight after subtraction: $newWeight');

          // Store as string with 3 decimal places, preserving negative sign
          String newWeightString = newWeight.toStringAsFixed(3);
          accounts[accountIndex]['weight'] = newWeightString;

          // Save updated accounts
          await prefs.setString('accounts', jsonEncode(accounts));

          print(
            'Account weight updated and saved: $selectedName - Old: $currentWeight, Subtracted: $totalPureWeight, New: $newWeightString',
          );

          // Verify it was saved correctly
          final verifyJson = prefs.getString('accounts');
          if (verifyJson != null) {
            final verifyAccounts = jsonDecode(verifyJson);
            final verifyAccount = verifyAccounts[accountIndex];
            print(
              'Verification - Saved weight value: ${verifyAccount['weight']}',
            );
          }
        } else {
  ...
  Future<void> _updateInventoryQuantities() async {
    ...
    if (itemsJson != null && itemsJson.isNotEmpty) {
      try {
        ...
        for (var issuedItem in goldItems) {
          String itemName = issuedItem['description'];
          int issuedQuantity = int.tryParse(issuedItem['set']) ?? 1;
          double issuedWeight = double.tryParse(issuedItem['gross']) ?? 0.0;
          ...
          if (index != -1) {
            ...
            int newQty = currentQty - issuedQuantity;
            ...
            double currentWeight = (inventoryItems[index]['weight'] ?? 0.0)
                .toDouble();
            double newWeight = currentWeight - issuedWeight;
          ...
          Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Icon(Icons.diamond, color: Colors.amber[300], size: 28),
                    SizedBox(width: 12.w),
                    Expanded(
                      child: Column(
                        ...
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 8.h),
                                Text(
                                  'Click "Add" button to issue items for alteration',
                                  style: TextStyle(
                                    fontSize: 14.sp,
